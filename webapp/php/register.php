...
if(isset($_POST['register'])&&isset($_POST['username'])&&isset($_POST['email'])&&isset($_POST['password'])&&isset($_POST['password2']))
{
    ...
    if(empty($username)|| empty($email)|| empty($password) || empty($password2))    {
        header ('location:../registration.php?error=empty_value') ;
    }
    ...
    elseif(preg_match("/^[a-zA-Z0-9]*%/")){
        header ('location:../registration.php?error=invalid_data') ;
        exit();
    }
    elseif($password!==$password2){
        header ('location:../registration.php?error=password_did_not_match') ;
        exit();
    }
    else{
                ...
                if(!mysqli_stmt_prepare($stmt,$query)){
                 ...
                 $resultCheck = mysqli_stmt_num_rows($stmt);
                             if($resultCheck > 0){
                               ...
                               header("location:../registration.php?error=username_already_exists");
                                exit();
                                }
                                    else{
                                        ...
                                        if(!mysqli_stmt_prepare($stmt,$query)){
                                            header("location:../registration.php?error=query");
                                            exit();
                                                                    }
                                            else{
